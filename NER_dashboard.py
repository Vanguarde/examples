...
# класс для подготовки кода для дэшборда (HTML код для вставки в дэшборд HTML для отображения (без необх устанавливать python))
class NER_dashboard:
    def __init__(self):
        self.colors = {}
        self.patterns = []
    ...
    def nlp_func(self, row):
        ...
        doc = self.nlp(row['news_for_reading'])
